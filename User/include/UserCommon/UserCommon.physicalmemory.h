...
#include <windows.h>
#include "UserCommon.function_ref.hpp"
...
public:
    ...
    {
        // OutputDebugStringA("WriteProcessMemory Start\n");
        while (Size > 0) 
        {
            size_t BlockSize = 0x1000 - (Address & 0xFFF);
            if (BlockSize > Size)
            ...
            if (!WritePhysicalMemory(PA, Buffer, BlockSize))  //<-----这里调用外部绑定的lambda函数
                return false;
        ...
        // OutputDebugStringA("WriteProcessMemory End\n");
        return true;
    }
};