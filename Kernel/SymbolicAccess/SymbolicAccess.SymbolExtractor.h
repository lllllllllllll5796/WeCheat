...
namespace symbolic_access
{
	...
	private:
		...
		eastl::vector<IMAGE_SECTION_HEADER> ReadSectionHeaders(uint32_t HeaderStream);

		eastl::vector<DbiModuleInfo> GetModulesInfo(StreamReader& Reader, size_t ModulesSize);
		...
		bool SkipSymbol(eastl::string_view SymbolName);

		void ReadOmaps(uint16_t OmapFromSourceStreamIndex, uint16_t OmapToSourceStreamIndex);

		eastl::optional<size_t> GetSymbolsOffset(size_t Offset, uint16_t Section);

		eastl::optional<size_t> TranslateOffset(size_t Offset, const eastl::vector<OMAP>& Omaps);
		...
		void InsertSymbol(StreamReader& Reader, SymbolsMap& Symbols)
		{
			...
			eastl::string nameCopy = name.data();
			...
			Symbols.insert(eastl::make_pair(std::move(nameCopy), *offset));
		}

		MsfReader& m_MsfReader;
		eastl::vector<IMAGE_SECTION_HEADER> m_SectionHeaders;
		eastl::vector<IMAGE_SECTION_HEADER> m_OriginalSectionHeaders;
		eastl::vector<OMAP> m_FirstOmaps;
		eastl::vector<OMAP> m_SecondOmaps;
	};
}