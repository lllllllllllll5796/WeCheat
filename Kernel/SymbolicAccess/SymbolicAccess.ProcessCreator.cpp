...
#include "SymbolicAccess.ScopedStackAttach.h"
...
#define PS_ATTRIBUTE_PROTECTION_LEVEL \
	...
	eastl::pair<ScopedHandle, ScopedHandle> ProcessCreator::CreateUmProcess(eastl::wstring_view ImagePath,
		eastl::wstring_view CommandLine, eastl::wstring_view CurrentDirectory, eastl::wstring_view DllPath)
	{
		...
		const auto buffer = eastl::make_unique<uint8_t[]>(finalParamtersSize);
		if (!buffer)
		{
		...
		return eastl::make_pair(ScopedHandle(processHandle), ScopedHandle(threadHandle));
	}
}