...
namespace symbolic_access
{
	...
	bool MsfReader::Initialize()
	{
		...
		eastl::vector<uint32_t> rootPages(numberOfRootPages);
		eastl::vector<uint32_t> rootIndexPages(numberOfRootIndexPages);
	...
	eastl::vector<char> MsfReader::GetStream(size_t Index)
	{
		const auto& stream = m_Streams[Index];
		eastl::vector<char> streamData(stream.PageIndices.size() * m_PageSize);
	...
	bool MsfReader::HasNullOffsets(const eastl::vector<uint32_t>& Offsets)
	{
		return eastl::any_of(Offsets.begin(), Offsets.end(), [](uint32_t Offset) { return Offset == 0; });
	}
}