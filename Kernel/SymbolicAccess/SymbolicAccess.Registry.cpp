...
namespace symbolic_access
{
	eastl::vector<uint8_t> GetRegistryData(eastl::wstring_view RegistryPath, eastl::wstring_view RegistryKeyName)
	{
		...
		auto buffer = eastl::vector<uint8_t>(keyInfoSize);
		...
		return eastl::vector<uint8_t>(buffer.begin() + keyInfo->DataOffset, buffer.begin() + keyInfo->DataOffset + keyInfo->DataLength);
	}
}