...
namespace symbolic_access
{
	...
	eastl::vector<SymbolsExtractor::IMAGE_SECTION_HEADER> SymbolsExtractor::ReadSectionHeaders(uint32_t HeaderStream)
	{
		eastl::vector<SymbolsExtractor::IMAGE_SECTION_HEADER> sectionsHeaders{};
	...
	eastl::vector<DbiModuleInfo> SymbolsExtractor::GetModulesInfo(StreamReader& Reader, size_t ModulesSize)
	{
		eastl::vector<DbiModuleInfo> modules{};
	...
	bool SymbolsExtractor::SkipSymbol(eastl::string_view SymbolName)
	{
		return SymbolName.empty() || 
			SymbolName.find("??_C@") == 0 || 
			SymbolName.find("__imp_") == 0 || 
			SymbolName.find("NULL_THUNK_DATA") != eastl::string_view::npos ||
			SymbolName.find("NULL_IMPORT_DESCRIPTOR") != eastl::string_view::npos;
	}
	...
	eastl::optional<size_t> SymbolsExtractor::GetSymbolsOffset(size_t Offset, uint16_t Section)
	{
		...
		if (m_FirstOmaps.size() && m_SecondOmaps.size())
		{
			...
			return eastl::optional<size_t>{};
		}

		return offset;
	}

	eastl::optional<size_t> SymbolsExtractor::TranslateOffset(size_t Offset, const eastl::vector<OMAP>& Omaps)
	{
		if (auto omapIter = eastl::find_if(Omaps.begin(), Omaps.end(), [&](OMAP Omap) {return Omap.SourceAddress > Offset; });
			omapIter != Omaps.end())
		{
