...
{
	StreamReader::StreamReader(const eastl::vector<char>& Stream) : m_Stream(Stream), m_StreamOffset(0)
	{
	}
	...
	size_t StreamReader::Read(void* Buffer, size_t Size)
	{
		...
		Size = std::min(m_Stream.size() - m_StreamOffset, Size);
	...
	eastl::string_view StreamReader::ReadString()
	{
		eastl::string_view result(m_Stream.data() + m_StreamOffset);
