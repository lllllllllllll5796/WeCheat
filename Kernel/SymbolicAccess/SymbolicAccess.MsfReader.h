...
namespace symbolic_access
{
	...
	public:
		...
		eastl::vector<char> GetStream(size_t Index);

	private:
		uint32_t GetNumberOfPages(uint32_t Size);

		bool HasNullOffsets(const eastl::vector<uint32_t>& Offsets);

		struct ContentStream
		{
			uint32_t Size;
			eastl::vector<uint32_t> PageIndices;
		};
		...
		uint32_t m_PageSize;
		eastl::vector<ContentStream> m_Streams;
