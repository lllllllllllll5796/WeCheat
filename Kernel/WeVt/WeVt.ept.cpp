...
namespace ept
{
	...
	bool setup_pml2_entry(mtrr_data const& mtrrs, __ept_state& ept_state, __ept_pde& entry, unsigned __int64 pfn)
	{
		...
		else
		{
			...
			{
#if ENABLE_TRACE
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Failed to allocate split buffer");
#endif
				return false;
			}
	...
	bool create_ept_page_table(__ept_state& ept_state)
	{
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Failed to allocate memory for PageTable");
#endif
			return false;
		}
		...
		for (int i = 0; i < 512; i++)
		{
			for (int j = 0; j < 512; j++)
			{
				...
				{
#if ENABLE_TRACE
					TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] setup_pml2_entry error");
#endif
					return false;
				}
			}
	...
	bool initialize(__ept_state& ept_state)
	{
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] ept_pointer is null");
#endif
			return false;
		}
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] create_ept_page_table error");
#endif
			return false;
		}
	...
	__ept_pde* get_pml2_entry(__ept_state& ept_state, unsigned __int64 physical_address)
	{
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] above 512GB is invalid");
#endif
			return nullptr;
		}
	...
	bool split_pml2(mtrr_data const& mtrrs, __ept_state& ept_state, void* pre_allocated_buffer, unsigned __int64 physical_address)
	{
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Invalid address passed");
#endif
			return false;
		}
	...
	bool hook_instruction_memory(__ept_state& ept_state, __ept_hooked_function_info* hooked_function_info, void* target_function, void* proxy_function, void** origin_function)
	{
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Function at page boundary");
#endif
			return false;
		}
		...
		unsigned __int64 target_cr3)
	{
			...
			if (hooked_page_info->pfn_of_hooked_page == GET_PFN(physical_address))
			{
				if (hooked_page_info->Options == EPTO_HOOK_FUNCTION)
				{
...
#if ENABLE_TRACE
							TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] There is no pre-allocated pool for trampoline");
#endif
							return false;
						}
					}
					...
					if (write_vmcall_instruction_to_memory(hooked_function_info, target_function, proxy_function, origin_function) == false)
					{
						...
						pool_manager::release_pool(hooked_function_info);
#if ENABLE_TRACE
						TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Hook failed");
#endif
						return false;
					}
		...
		if (is_page_splitted(ept_state, physical_address) == false)
		{
			...
			{
#if ENABLE_TRACE
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] There is no preallocated pool for split");
#endif
				return false;
			}
...
#if ENABLE_TRACE
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Split failed");
#endif
				return false;
			}
		}
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] Failed to get PML1 entry of the target address");
#endif
			return false;
		}
		...
		{
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] There is no preallocated pool for hooked page info");
#endif
			return false;
		}
	...
	bool hook_function(__ept_state& ept_state, void* target_function/*被挂钩的函数地址*/, void* proxy_function/*新函数地址*/, void** origin_function)
	{
		...
		if (hooked_function_info == nullptr)
		{
			pool_manager::release_pool(hooked_page_info);
#if ENABLE_TRACE
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[-] There is no preallocated pool for hooked function info");
#endif
			return false;
		}
				...
				(guest_physical_adddress < eptWatchList[ID].PhysicalAddress + eptWatchList[ID].Size))
			{
				...
				if (canBreak) //判断能否进行中断
				{
					...
					if (newRIP)
					{
						...
						hv::vmwrite(GUEST_INTERRUPTIBILITY_STATE, 1);  //blocking by STI 阻塞
					}
				}
			}
