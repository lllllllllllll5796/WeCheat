...
namespace VT_Debugger
{
	using namespace VT_Util;
	using namespace VT_InterruptHook;
	using namespace VmxHelper;
	BOOL bHookedInterrupts = FALSE;
#ifdef AMD64 
	extern "C" void interrupt1_asmentry(void); //declared in debuggera.asm
#else
	...
	void debugger_initialize(void)
	{
		...
		ImpCall(KeInitializeEvent, &debugger_event_WaitForDebugEvent, SynchronizationEvent, FALSE);

		//LOG_DEBUG("DebuggerState.fxstate=%p\n", DebuggerState.fxstate);

		StackCount = getCpuCount() * 4;
		Stacks = (PSavedStack*)ImpCall(ExAllocatePool, (POOL_TYPE)(NonPagedPool | POOL_ZERO_ALLOCATION), StackCount * sizeof(PSavedStack));

		if (Stacks && ImpCall(MmIsAddressValid, Stacks))
		{
			int i = 0;
			for (i = 0; i < StackCount; i++)
			{
				Stacks[i] = (PSavedStack)ImpCall(ExAllocatePool, (POOL_TYPE)(NonPagedPool | POOL_ZERO_ALLOCATION), sizeof(SavedStack));
			}
		}
	}
	...
	void debugger_shutdown(void)
	{
		if (Stacks)
		{
			...
			for (i = 0; i < StackCount; i++)
			{
				if (Stacks[i])
				{
					ImpCall(ExFreePool,Stacks[i]);
					Stacks[i] = NULL;
				}
			}

			ImpCall(ExFreePool,Stacks);
			Stacks = NULL;
		}
	}
		...
		//called in passive mode
	{
		if (Stacks)
		{
			KIRQL oldIRQL = ImpCall(KeRaiseIrqlToDpcLevel);
			...
			newStacks = (PSavedStack*)ImpCall(ExAllocatePool, NonPagedPool, newStackCount * sizeof(PSavedStack));
			...
			if (newStacks)
			{
				...
				{
					newStacks[i] = (PSavedStack)ImpCall(ExAllocatePool, NonPagedPool, sizeof(SavedStack));
					if (newStacks[i])
						RtlZeroMemory(newStacks[i], sizeof(SavedStack));
					else
					{
						ImpCall(ExFreePool,newStacks);
						csLeave(&StacksCS);
	...
	{
		//LOG_DEBUG("Unhooking int1 for this cpu\n");
	...
	{
		int result = TRUE;
		//LOG_DEBUG("Hooking int1 for cpu %d\n", cpunr());

		result = inthook_HookInterrupt(1, getCS() & 0xfff8, (QWORD)interrupt1_asmentry, &Int1JumpBackLocation);

#ifdef AMD64
		if (result)
		{
			;//LOG_DEBUG("hooked int1. Int1JumpBackLocation=%x:%llx\n", Int1JumpBackLocation.cs, Int1JumpBackLocation.eip);
		}
		else
		{
			//LOG_DEBUG("Failed hooking interrupt 1\n");
			return result;
		}
#endif
		...
		if (DebuggerState.globalDebug)
		{
			...
			//LOG_DEBUG("Setting GD bit for cpu %d\n",cpunr());
		...
		{
			//LOG_DEBUG("Enabling LBR logging. IA32_DEBUGCTL was %x\n", __readmsr(0x1d9));
			__writemsr(0x1d9, __readmsr(0x1d9) | 1);
			//LOG_DEBUG("Enabling LBR logging. IA32_DEBUGCTL is  %x\n", __readmsr(0x1d9));
		}*/
	...
	void debugger_setStoreLBR(BOOL state)
	{
		...
		/*
		//if (state)
		//	LOG_DEBUG("Setting storeLBR to true\n");
		//else
		//	LOG_DEBUG("Setting storeLBR to false\n");
		...
		//LOG_DEBUG("Because your cpu_model=%d I think that your storeLBR_max=%d\n", cpu_model, DebuggerState.storeLBR_max);
		*/
	...
	int debugger_setGlobalDebugState(BOOL state)
		...
		//在调试之前调用此函数，如果已经调试，则用户必须为每个cpu调用此函数
	{
		//LOG_DEBUG("Debugger_SetGlobalDebugState(%d)\n",state);
	    if (state)
		...
		if (inthook_isHooked(1))
		{
			...
			//LOG_DEBUG("Int 1 is hooked,%ssetting GD\n",(state ? "":"un"));
			//LOG_DEBUG("oldEpilogueState=%d\n",oldEpilogueState);
			//debugger_setInitialFakeState();
	...
	BOOL debugger_stopDebugging(void)
	{
	    int i;
		...
		forEachCpuPassive((PF)debugger_removeHookForCurrentCPU, 0);
	...
	int debugger_setGDBreakpoint(int breakpointnr, QWORD Address, BreakType bt, BreakLength bl)
		/*
	...
	NTSTATUS debugger_waitForDebugEvent(ULONG timeout)
	{
		...
		if (timeout == 0xffffffff) //infinite wait
			r = ImpCall(KeWaitForSingleObject, &debugger_event_WaitForDebugEvent, UserRequest, KernelMode, TRUE, NULL);
		else
			r = ImpCall(KeWaitForSingleObject, &debugger_event_WaitForDebugEvent, UserRequest, KernelMode, TRUE, &wait);
	...
	NTSTATUS debugger_continueDebugEvent(BOOL handled)
		...
		*/
	{
		//LOG_DEBUG("debugger_continueDebugEvent\n");

		DebuggerState.handledlastevent = handled;
		ImpCall(KeSetEvent, &debugger_event_WaitForContinue, 0, FALSE);
		...
		//Interrupts are enabled and should be at passive level, so taskswitching is possible
	{
		...
		//LOG_DEBUG("breakpointHandler for kernel breakpoints\n");

#ifdef AMD64
	//LOG_DEBUG("cs=%x ss=%x ds=%x es=%x fs=%x gs=%x\n",getCS(), getSS(), getDS(), getES(), getFS(), getGS());

	//LOG_DEBUG("fsbase=%llx gsbase=%llx gskernel=%llx\n", readMSR(0xc0000100), readMSR(0xc0000101), readMSR(0xc0000102));

	//LOG_DEBUG("rbp=%llx\n", getRBP());

	//LOG_DEBUG("gs:188=%llx\n", __readgsqword(0x188));
	//LOG_DEBUG("causedbyDBVM=%d\n", causedbyDBVM);
#endif

		if (ImpCall(KeGetCurrentIrql) == 0)
		{
			//crititical section here
			if ((stackpointer[si_cs] & 3) == 0)
			{
				//LOG_DEBUG("Going to wait in a kernelmode routine\n");
			}
			...
			{
				r = ImpCall(KeWaitForSingleObject, &debugger_event_CanBreak, Executive, KernelMode, FALSE, NULL);
				//check r and handle specific events

				//LOG_DEBUG("Woke up. r=%x\n",r);

			}

			if ((stackpointer[si_cs] & 3) == 0)
			{
				//LOG_DEBUG("Woke up in a kernelmode routine\n");
			}
			...
			DebuggerState.LastThreadID = ImpCall(PsGetCurrentThreadId);
			...
			ImpCall(KeSetEvent,&debugger_event_WaitForDebugEvent, 0, FALSE);
			...
			while (1)
			{
				...
				{
					s = ImpCall(KeWaitForSingleObject,&debugger_event_WaitForContinue, Executive, KernelMode, FALSE, NULL);
					//LOG_DEBUG("KeWaitForSingleObject=%x\n",s);		
				}
				...
				if (s == STATUS_SUCCESS)
				{
					...
					{
						//LOG_DEBUG("handledlastevent=TRUE");
						handled = 1;
					}
					else
						handled = 0;
			...
			ImpCall(KeSetEvent,&debugger_event_CanBreak, 0, FALSE);

			//LOG_DEBUG("Returning after a wait. handled=%d and eflags=%x\n",handled, stackpointer[si_eflags]);
			if ((stackpointer[si_cs] & 3) == 0) //check rpl of cs
			{
				//LOG_DEBUG("and in kernelmode\n");
			}
		...
		{
			//LOG_DEBUG("Breakpoint wasn't at passive level. Screw this, i'm not going to break here\n");
	...
	{
		HANDLE CurrentProcessID = ImpCall(PsGetCurrentProcessId);
		//LOG_DEBUG("INT 1 进程PID:%d\r\n", CurrentProcessID);
...
// 		LOG_DEBUG("interrupt1_handler(%p). DR6=%x (%x) DR7=%x %x:%p\n",
// 			Interrupt1_handler, originaldr6, 
// 			Debugger_Dr6_GetValueDword(),   //DR6=ffff4ff0
// 			Debugger_Dr7_GetValueDword(),   //DR7=400
// 			stackpointer[si_cs], 
// 			(void*)(stackpointer[si_eip]));

		//check if this break should be handled or not

		if (DebuggerState.globalDebug)
		{
			//LOG_DEBUG("DebuggerState.globalDebug=TRUE\n");
			...
			if (_dr6.BD)
			{
				...
				if ((stackpointer[si_cs] & 3) == 0)
				{
					...
					{
						((VT_Util::EFLAGS*)&stackpointer[si_eflags])->RF = 1; //repeat this instruction and don't break
						return 2;
					}
					...
					if (instruction[instructionPointer + 1] == 0x21)
					{
						...
						if (instruction[instructionPointer + 1] == 0x23)
						{
							...
							case 4:
							case 6:
								//LOG_DEBUG("Setting dr6 to %x (was %x)\n", gpvalue, DebuggerState.FakedDebugRegisterState[cpunr()].DR6);							
								_dr6 = *(DebugReg6*)&gpvalue;

								//if (_dr6.BD) LOG_DEBUG("Some code wants to set the BD flag to 1\n");
						...
						{
							//LOG_DEBUG("Some unknown instruction accessed the debug registers?\n");
							//if (CurrentProcessID==(HANDLE)(UINT_PTR)DebuggerState.debuggedProcessID)
							//	LOG_DEBUG("Happened inside the target process\n");

							//LOG_DEBUG("interrupt1_handler dr6=%x (original=%x) dr7=%d\n",_dr6, originaldr6, _dr7);
							//LOG_DEBUG("eip=%x\n",stackpointer[si_eip]);
						}
				...
				{
					//LOG_DEBUG("DR6.BD == 1 in USERMODE! WTF\n");
					_dr6.BD = 0;
					debugger_dr6_setValue(*(UINT_PTR*)&_dr6);
					DebuggerState.FakedDebugRegisterState[cpunr()].DR6 = debugger_dr6_getValueDword();
				}
		...
		{
			LOG_DEBUG("isSteppingTillClear\r\n");
			if ((((PEFLAGS)&stackpointer[si_eflags])->IF == 0) || (ImpCall(KeGetCurrentIrql) != PASSIVE_LEVEL))
			{
		...
		{
			LOG_DEBUG("DebuggerState.isDebugging\n");
			...
			if (CurrentProcessID == (HANDLE)(UINT_PTR)DebuggerState.debuggedProcessID)
			{
				...
				if ((((PEFLAGS)&stackpointer[si_eflags])->IF == 0) || (ImpCall(KeGetCurrentIrql) != PASSIVE_LEVEL))
				{
					...
					//LOG_DEBUG("int1 at unstoppable location");
					if (!KernelCodeStepping)
					{
						((PEFLAGS)&stackpointer[si_eflags])->TF = 0; //just give up stepping
					//	LOG_DEBUG("Quitting this");
					}
					else
					{
						//	LOG_DEBUG("Stepping until valid\n");
				...
				//LOG_DEBUG("CurrentProcessID==(HANDLE)(UINT_PTR)DebuggerState.debuggedProcessID\n");
				...
				//LOG_DEBUG("BP in target process\n");
	...
	/*
				...
				{
					//LOG_DEBUG("Breakpoint while interrupts are disabled: %x\n",stackpointer[si_eip]);
			...
			*/

				{
					...
					//LOG_DEBUG("calling breakpointHandler_kernel\n");
	...
	/*
					...
					*/
					...
					else
					{
						...
						/*if (getDR7() != oldDR7)
						{
							LOG_DEBUG("Something changed DR7. old=%llx new=%llx\n",oldDR7, getDR7());
						}*/
						...
						{
							//	LOG_DEBUG("WTF? GD is 1 in currentdebugregs[5]: %llx\n", currentdebugregs[5]);
						}
						else
			...
			else
			{
				...
				{
					//LOG_DEBUG("That thing that shouldn\'t happen often happened\n");
				...
				//LOG_DEBUG("Returning unhandled. DR6=%x", debugger_dr6_getValueDword());
	...
	//1号中断被重定向到这里了
	extern "C" int interrupt1_centry(UINT_PTR * stackpointer) //code segment 8 has a 32-bit stackpointer
	{
		...
		QWORD naddress;
		//放开这里就可以看到单步时候的打印
		//LOG_DEBUG("Interrupt1_CEntry Cpunr=%d Esp=0x%p\n",Cpunr(), GetRSP());
		//LOG_DEBUG("步过后的 RIP:0x%p\r\n", stackpointer[si_eip]);
...
#endif
		//LOG_DEBUG("naddress:%p\r\n", naddress);  //KiDebugTrapOrFault的地址
