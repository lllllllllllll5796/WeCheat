...
#include "CE.NoExceptions.h"

namespace MemScan
{
	using namespace DBKUtil;
	using namespace VmxHelper;
	using namespace VmxOffLoad;
	using namespace NoExceptions;
	...
	//----------------------
	static UINT_PTR KnownPageTableBase = 0;
	...
	*/
	...
	BOOLEAN IsAddressSafe(UINT_PTR StartAddress)
	{
		...
		if (loadedbydbvm)
		{
			...
			if (ImpCall(MmIsAddressValid, (PVOID)StartAddress))
			{
				//LOG_DEBUG("Address:%p\r\n", StartAddress);
			...
			//LOG_DEBUG("IsAddressSafe dbvm-mode: lastError=%p\n", lasterror);
...

		{
			...
			else
			{
				...
				physical.QuadPart = 0;
				physical = ImpCall(MmGetPhysicalAddress, (PVOID)StartAddress);
	...
	UINT_PTR getPEThread(UINT_PTR threadid)
	{
		...
		if (ImpCall(PsLookupThreadByThreadId, (PVOID)(UINT_PTR)threadid, &selectedthread) == STATUS_SUCCESS)
		{
	...
	BOOLEAN WriteProcessMemory(DWORD PID, PEPROCESS PEProcess, PVOID Address, DWORD Size, PVOID Buffer)
	{
		...
		{
			//LOG_DEBUG("WriteProcessMemory:Getting PEPROCESS\n");
			if (!NT_SUCCESS(ImpCall(PsLookupProcessByProcessId, (PVOID)(UINT_PTR)PID, &selectedprocess)))
				return FALSE; //couldn't get the PID
		
			 //LOG_DEBUG("Retrieved peprocess");  
		}
		...
		__try
		{
			...
			ImpCall(KeAttachProcess, (PEPROCESS)selectedprocess);
		...
		if (PEProcess == NULL) //no valid peprocess was given so I made a reference, so lets also dereference
			ImpCall(ObfDereferenceObject,selectedprocess);
	...
	static UINT64 maxPhysAddress = 0;
	...
	UINT64 getMaxPhysAddress(void)
	{
		if (maxPhysAddress == 0)
		{
			int physicalbits;
			int r[4];
			KernelIntrin__cpuid(r, 0x80000008);
	...
	NTSTATUS ReadPhysicalMemory(char* startaddress, UINT_PTR bytestoread, void* output)
	{
		...
		WCHAR*			  physmemName = (WCHAR*)oxorany(L"\\device\\physicalmemory");
		UCHAR* memoryview;
		NTSTATUS		ntStatus = STATUS_UNSUCCESSFUL;
		PMDL			outputMDL;

		//LOG_DEBUG("ReadPhysicalMemory(%p, %d, %p)\n", startaddress, bytestoread, output);

		if (((UINT64)startaddress > getMaxPhysAddress()) || ((UINT64)startaddress + bytestoread > getMaxPhysAddress()))
		{
			LOG_DEBUG("Invalid physical address\n");
			return ntStatus;
		}

		outputMDL = ImpCall(IoAllocateMdl, output, (ULONG)bytestoread, FALSE, FALSE, NULL);
		__try
		{
			ImpCall(MmProbeAndLockPages, outputMDL, KernelMode, IoWriteAccess);
		}
		__except (1)
		{
			ImpCall(IoFreeMdl, outputMDL);
			return STATUS_UNSUCCESSFUL;
		}

		__try
		{
			ImpCall(RtlInitUnicodeString, &physmemString, physmemName);

			InitializeObjectAttributes(&attributes, &physmemString, OBJ_CASE_INSENSITIVE, NULL, NULL);
			ntStatus = ImpCall(ZwOpenSection, &physmem, SECTION_ALL_ACCESS, &attributes);
			if (ntStatus == STATUS_SUCCESS)
			{
				...
				//LOG_DEBUG("ReadPhysicalMemory:viewBase.QuadPart=%x\n", viewBase.QuadPart);

				ntStatus = ImpCall(ZwMapViewOfSection,
				...
				if ((ntStatus == STATUS_SUCCESS) && (memoryview != NULL))
				{
					...
					if (toread)
					{
						//__try
						{
							...
							ImpCall(ZwUnmapViewOfSection, NtCurrentProcess(), memoryview);
						}
// 						__except (1)
// 						{
// 							LOG_DEBUG("Failure mapping physical memory\n");
// 						}
					}
				}
				else
				{
					//LOG_DEBUG("ReadPhysicalMemory error:ntStatus=%x\n", ntStatus);
				}

				ImpCall(ZwClose, physmem);
			};
		}
		__except (1)
		{
			LOG_DEBUG("Error while reading physical memory\n");
		}

		ImpCall(MmUnlockPages,outputMDL);
		ImpCall(IoFreeMdl,outputMDL);
		return ntStatus;
	}
	...
	UINT_PTR getPageTableBase()
	{
		...
		if (KnownPageTableBase == 0)
		{
			...
			if (ImpCall(RtlGetVersion, &v))
			{
				LOG_DEBUG("RtlGetVersion failed\n");
				return 0;
			}

			if ((v.dwMajorVersion >= 10) && (v.dwBuildNumber >= 14393))  //从1607开始页表基地址开始开始不固定了
			{
				PHYSICAL_ADDRESS a;
				PVOID r;
				a.QuadPart = getCR3() & 0xFFFFFFFFFFFFF000ULL;
				r = ImpCall(MmGetVirtualForPhysical, a); //if this stops working, look for CR3 in the pml4 table
	...
	BOOL walkPagingLayout(PEPROCESS PEProcess, UINT_PTR MaxAddress, PRESENTPAGECALLBACK OnPresentPage)
	{
		...
		__try
		{
			ImpCall(KeAttachProcess,(PEPROCESS)PEProcess);
			//__try
			{
				...
				{
					//LOG_DEBUG("currentAddress=%p\n", currentAddress);
					lastAddress = currentAddress;


					PPTE = (PTEStruct*)((UINT_PTR)(((currentAddress & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase));
					PPDE = (PTEStruct*)((UINT_PTR)((((UINT_PTR)PPTE) & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase);
					PPDPE = (PTEStruct*)((UINT_PTR)((((UINT_PTR)PPDE) & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase);
					PPML4E = (PTEStruct*)((UINT_PTR)((((UINT_PTR)PPDPE) & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase);

					if (PTESize == 8)
						PPDPE = (PTEStruct*)(((((UINT_PTR)PPDE) & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase);
					else
						PPDPE = 0;

#ifdef AMD64
					PPML4E = (PTEStruct*)(((((UINT_PTR)PPDPE) & 0xFFFFFFFFFFFFULL) >> 12) * PTESize + pagebase);
#else
					(UINT_PTR)PPML4E = 0;
#endif
		...
		__except (1)
		{
			//LOG_DEBUG("Excepion while walking the paging layout\n");
			return FALSE;
		}
	...
	static PPENTRY AccessedList = NULL;
	static int AccessedListSize = 0;
	...
	void StoreAccessedRanges(UINT_PTR StartAddress, UINT_PTR EndAddress, PTEStruct* pageEntry)
	{
		if (pageEntry->A)
		{
			...
			else
			{
				//insert
				PPENTRY e;
				e = (PPENTRY)ImpCall(ExAllocatePool, PagedPool, sizeof(PENTRY));
	...
	int getAccessedPageList(PPRANGE List, int ListSizeInBytes)
	{
		...
		//	LOG_DEBUG("getAccessedPageList\n");

		while (e)
		{
			if (i >= maxcount)
			{
				//LOG_DEBUG("%d>=%d", i, maxcount);
				break;
			}
			...
			*protection = 0;
			...
			while (1)
			{
				...
				if (pml4entry->P == 0)
				{
					...
					{
						if ((ri >= 512) || (ri < 0))
							ImpCall(DbgBreakPointWithStatus,ri);
	...
	BOOLEAN GetMemoryRegionData(DWORD PID, PEPROCESS PEProcess, PVOID mempointer, ULONG* regiontype, UINT_PTR* memorysize, UINT_PTR* baseaddress)
	{
		...
		{
			LOG_DEBUG("GetMemoryRegionData failed because pagebase == 0\n");
			return FALSE;
		}
		...
		*regiontype = 0;
		...
		__except (1)
		{
			//LOG_DEBUG("Exception in GetMemoryRegionData\n");
			//LOG_DEBUG("mempointer=%p\n", mempointer);
		}

		return FALSE;
	}
	
	NTSTATUS WritePhysicalMemory(char* startaddress, UINT_PTR bytestowrite, void* pinp)
	{
		NTSTATUS		ntStatus = STATUS_UNSUCCESSFUL;
		HANDLE			physmem;
		UNICODE_STRING	physmemString;
		...
		if (ntStatus == STATUS_SUCCESS)
		{
			...
			viewBase.QuadPart = (ULONGLONG)(startaddress);  //写入的物理地址

			length = 0x2000;//pinp->bytestoread;
			towrite = (UINT_PTR)bytestowrite;
			...
			if (ntStatus == STATUS_SUCCESS)
			{
				...
				RtlCopyMemory(&memoryview[offset], &pinp2[16], towrite);

				ImpCall(ZwUnmapViewOfSection,
					NtCurrentProcess(), //processhandle
					memoryview);
			}
