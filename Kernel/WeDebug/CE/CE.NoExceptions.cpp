...
namespace NoExceptions
{
...
#ifdef AMD64
	extern "C" void NoException14(void); //declared in debuggera.asm
	extern "C" int ExceptionlessCopy_Internal(PVOID destination, PVOID source, int size);
#else
	extern "C" void __cdecl NoException14(void); //declared in debuggera.asm
	extern "C" int __cdecl ExceptionlessCopy_Internal(PVOID destination, PVOID source, int size);
#endif
	...
	BOOL NoExceptions_Enter()
	{
		...
		__try
		{
			...
			{
				//initialize the list
				MaxCPUCount = (int)ImpCall(KeQueryActiveProcessorCount,NULL);
				cpustate = (PCPUSTATE)ImpCall(ExAllocatePool,NonPagedPool, MaxCPUCount * sizeof(CPUSTATE));
				...
				if (cpustate)
				{
					...
					{
						cpustate[i].NoExceptionVectorList = (VT_InterruptHook::PINT_VECTOR)ImpCall(ExAllocatePool, NonPagedPool, 256 * sizeof(VT_InterruptHook::INT_VECTOR));
						if (cpustate[i].NoExceptionVectorList)
						{
							RtlZeroMemory(cpustate[i].NoExceptionVectorList, 256 * sizeof(VT_InterruptHook::INT_VECTOR));
						}
						else
						{
							...
							int j;
							for (j = i - 1; i >= 0; i--)
								ImpCall(ExFreePool, cpustate[i].NoExceptionVectorList);
			...
			old = ImpCall(KfRaiseIrql, HIGH_LEVEL);
			cpunr = ImpCall(KeGetCurrentProcessorNumber);
			...
			if (cpustate[cpunr].OriginalIDT.wLimit == 0)
			{
				...
				UINT_PTR newAddress;
				KernelIntrin__sidt(&cpustate[cpunr].OriginalIDT);
...
#endif
			KernelIntrin__lidt(&cpustate[cpunr].ModdedIDT);
		...
		__except (1)
		{
			LOG_DEBUG("Exception during NoExceptions_Enter. Figures");
		}
	...
	void NoExceptions_Leave()
	{
		...
		int cpunr = ImpCall(KeGetCurrentProcessorNumber);

		//restore the IDT
		KernelIntrin__lidt(&cpustate[cpunr].OriginalIDT);
#ifdef AMD64	
		_enable();
#else
		__asm {
...
#endif
		ImpCall(KeLowerIrql, cpustate[cpunr].entryIRQL);
	}
