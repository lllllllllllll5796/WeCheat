...
--*/
{
    ...
    // 现在不再会出现更多引用。
    //
    for (Process = (PEPROCESS)Global::PsGetNextProcess(NULL); Process != NULL; Process = (PEPROCESS)Global::PsGetNextProcess((PEPROCESS)Process))
    {
...
BOOLEAN IsDebugTargetProcess(IN PEPROCESS Process, OUT PDEBUG_PROCESS* DebugProcess)
{
	...
	*DebugProcess = NULL;
	...
	while (ListHead != NextEntry)
	{
		...
		if (entry)
		{
			if (entry->Process == (_EPROCESS*)Process)
			{
				*DebugProcess = entry;
				result = TRUE;
				break;
...
EXTERN_C
NTSTATUS Fake_NtCreateDebugObject(OUT PHANDLE DebugHandle,
...
NTSTATUS
Fake_NtSetInformationDebugObject(
...
VOID InsertDebuggerList(PDEBUGGER_TABLE_ENTRY Debugger)
{
    if (Debugger)
    {
        ExAcquireFastMutex(&Global::g_DebuggerList.Mutex);
        InsertNode(&Global::g_DebuggerList.list_entry, &Debugger->list_entry);
        ExReleaseFastMutex(&Global::g_DebuggerList.Mutex);
    }
}
...
BOOLEAN IsDebugger(PEPROCESS Process)
{
    ...
    PLIST_ENTRY ListHead = &Global::g_DebuggerList.list_entry.ListHead;
    PLIST_ENTRY NextEntry = ListHead->Flink;
    while (ListHead != NextEntry)
    {
        PDEBUGGER_TABLE_ENTRY entry = CONTAINING_RECORD(NextEntry, DEBUGGER_TABLE_ENTRY, list_entry);
        if (entry && entry->dwPid == (DWORD)(ULONG_PTR)pid)
        {
            result = TRUE;
            break;
        }
...
{
	size_t ptr_ImageFileName = (size_t)Process + Offset::EProcess::ImageFileName;
	LOG_DEBUG("[+] process: %s pid: %p\r\n", (char*)ptr_ImageFileName, PsGetProcessId(Process));
}
...
VOID InsertDebugProcessList(PDEBUG_PROCESS debug_process)
{
    ...
    {
        ExAcquireFastMutex(&Global::g_DebugProcessList.Mutex);
        InsertNode(&Global::g_DebugProcessList.list_entry, &debug_process->list_entry);
        ExReleaseFastMutex(&Global::g_DebugProcessList.Mutex);
    }
}
...
//{
//    Global::g_DebuggerList->Lock();
//    __try
//    {
//        for (DWORD i = 0; i < Global::g_DebuggerList->Count(); i++)
//        {
//            PDEBUGGER_DATA tmp = (PDEBUGGER_DATA)Global::g_DebuggerList->Get(i);
//            if (tmp)
//            {
//                delete tmp;
//            }
...
//    __finally
//    {
//        Global::g_DebuggerList->UnLock();
//    }
//    delete Global::g_DebuggerList;
//}
...
VOID ReleaseDebugProcessList()
{
    ...
    {
        LOG_DEBUG("[-] ReleaseDebugProcessList exception\r\n");
    }
}

//移除元素
// DeleteDebugProcess is implemented in WeDebug.DbgkApi.cpp
//设置调试对象
BOOLEAN SetDebugTargetProcess(PEPROCESS Process, PDEBUG_OBJECT DebugObject)
{
    ...
    if (IsDebugger(PsGetCurrentProcess()))
    {
        ...
        entry->Process = ( _EPROCESS*)Process;
...
//判断目标进程是否是被调试的进程
// IsDebugTargetProcess is implemented in WeDebug.DbgkApi.cpp
EXTERN_C
NTSTATUS Fake_DbgkOpenProcessDebugPort(IN PEPROCESS Process,
...
#pragma optimize("", off)
//EXTERN_C
//VOID Fake_DbgkCreateThread(IN PETHREAD Thread)
//{
//    Debugport_Struct s;
//    PDEBUG_OBJECT Port;
//    PDEBUG_PROCESS DebugProcess;
//    size_t kthread_base = (size_t)Thread + Offset::EThread::Tcb;
//    size_t ApcState = kthread_base + Offset::KThread::ApcState;
//    size_t ptr_Process = ApcState + Offset::KApcState::Process;
//
//    if (IsDebugTargetProcess(*(PEPROCESS*)ptr_Process, &DebugProcess))
//    {
//        //DbgPrint("Fake_DbgkCreateThread ptr_Process: %p", *(PEPROCESS*)ptr_Process);
//        Port = DebugProcess->DebugObject;
//    }
//    else
//    {
//        size_t ptr_DebugPort = *(size_t*)ptr_Process + Offset::EProcess::DebugPort;
//        Port = *(PDEBUG_OBJECT*)ptr_DebugPort;
//    }    
//
...
EXTERN_C
VOID Fake_PspExitThread(_In_ NTSTATUS ExitStatus)
{
    Debugport_Struct s;
    PDEBUG_OBJECT Port;
    PDEBUG_PROCESS DebugProcess;
    PEPROCESS Process = (PEPROCESS)PsGetCurrentProcess();
    ...
    {
        size_t ptr_DebugPort = (size_t)Process + Offset::EProcess::DebugPort;
        Port = *(PDEBUG_OBJECT*)ptr_DebugPort;
    }

    s.id = 0xfbbd6252520ea3d5;  //PspExitThread 经16位md5哈希摘要
    s.DebugPort = (size_t)Port;
    Global::Original_PspExitThread(ExitStatus);
}
#pragma optimize("", on)
...
VOID Fake_DbgkCreateThread(IN PETHREAD Thread)
{
    ...
    PWIN11_PS_SYSTEM_DLL_INFO DllInfo;
    PSECTION Section;
    ...
    Process = Global::PsGetCurrentProcessByThread(Thread);
    size_t ptr_WoW64Process = (size_t)Process + Offset::EProcess::WoW64Process;
    isWoW64Process = WeDebug_IsWow64Process(Process);

    size_t ptr_Flags = (size_t)Process + Offset::EProcess::Flags;

    //将ImageNotifyDone和CreateReported置位
    OldFlags.Flags = InterlockedOr((volatile LONG*)ptr_Flags, 0x400001u);

    if (!(OldFlags.ImageNotifyDone) && (*Global::PspNotifyEnableMask & 1 || *Global::PerfGlobalGroupMask & 4))
    {
        ImageInfoEx.Size = 0;
        ImageInfoEx.ImageInfo.Properties = 0;
        ImageInfoEx.ImageInfo.ImageAddressingMode = IMAGE_ADDRESSING_MODE_32BIT;
        size_t ptr_SectionBaseAddress = (size_t)Process + Offset::EProcess::SectionBaseAddress;
        ...
        Global::PsReferenceProcessFilePointer(Process, &FileObject);
        ...
        Global::PsCallImageNotifyRoutines(ImageFileName, Process, &ImageInfoEx, FileObject);  //通知模块回调
        if (ImageFileName)
        {
        ...
        {
            DllInfo = (PWIN11_PS_SYSTEM_DLL_INFO)Global::PsQuerySystemDllInfo(i);

            if (DllInfo && (i <= 0 || ((DllInfo->Flags & 8) && *(size_t*)ptr_WoW64Process && i == Global::PsWow64GetProcessNtdllType(Process))))
            {
                ImageInfoEx.ImageInfo.Properties = 0;
                ImageInfoEx.ImageInfo.ImageAddressingMode = IMAGE_ADDRESSING_MODE_32BIT;
                ...
                Section = Global::PspReferenceSystemDll(SystemDllData->SystemDll);
                ControlArea = (PCONTROL_AREA)Global::MiSectionControlArea(Section);
                if (Global::MiReferenceControlAreaFileWithTag)
                {
                    FileObject = (PFILE_OBJECT)Global::MiReferenceControlAreaFileWithTag(ControlArea, 'tlfD');
                }
                else
                {
                    FileObject = Global::MiReferenceControlAreaFile(ControlArea);
                }
                if (Section)
                {
                    Global::ObFastDereferenceObject(&SystemDllData->SystemDll->DllSection, Section);
                }
                Global::PsCallImageNotifyRoutines(&DllInfo->DllPath, Process, &ImageInfoEx, FileObject);
                ObDereferenceObject(FileObject);
            }
        }
    }
    ...
    if (IsDebugTargetProcess((PEPROCESS)Process, &DebugProcess))
    {
    ...
    if (!(OldFlags.CreateReported))
    {
        ...
        size_t ptr_SectionObject = (size_t)Process + Offset::EProcess::SectionObject;
        CreateProcess->FileHandle = Global::DbgkpSectionToFileHandle(*(PVOID*)ptr_SectionObject);
        size_t ptr_SectionBaseAddress = (size_t)Process + Offset::EProcess::SectionBaseAddress;
        ...
        /* Send the message */
        Global::DbgkpSendApiMessage((PEPROCESS)Process, FALSE, &ApiMessage);
        ...
        Global::DbgkSendSystemDllMessages(NULL, NULL, &ApiMessage);
    }
    else
    {
        /* Otherwise, do it just for the thread */
        CreateThread->SubSystemKey = 0;
        size_t ptr_Win32StartAddress = (size_t)Thread + Offset::EThread::Win32StartAddress;
        ...
        /* Send the message */
        Global::DbgkpSendApiMessage((PEPROCESS)Process, TRUE, &ApiMessage);
    }

    size_t ptr_SameThreadPassiveFlags = (size_t)Thread + Offset::EThread::SameThreadPassiveFlags;
    if (*(unsigned long*)ptr_SameThreadPassiveFlags & 16)
        Global::DbgkpPostModuleMessages(Process, Thread, NULL);
}

NTSTATUS Fake_DbgkpQueueMessage(IN PEPROCESS Process,
    IN PETHREAD Thread,
    ...
    IN PDEBUG_OBJECT TargetObject OPTIONAL)
{
    ...
    if (NewEvent)
    {
        ...
        DebugEvent->BackoutThread = (PETHREAD)PsGetCurrentThread();
    ...
    else
    {
        ...
        case DbgKmCreateProcessApi:
        {
            ...
            //size_t ptr_SkipCreationMsg = (size_t)Thread + Offset::EThread::CrossThreadFlags;
        ...
        case DbgKmExitProcessApi:
        {
            ...
            //size_t ptr_SkipTerminationMsg = (size_t)Thread + Offset::EThread::SkipTerminationMsg;
            //if (*(unsigned long*)ptr_SkipTerminationMsg) DebugObject = NULL;
            break;
        }
        case DbgKmLoadDllApi:
        {
            //size_t ptr_SkipCreationMsg = (size_t)Thread + Offset::EThread::SkipCreationMsg;
    ...
    //DbgBreakPoint();
    size_t ptr_Cid = (size_t)Thread + Offset::EThread::Cid;
...
//转发异常
BOOLEAN
Fake_DbgkForwardException(IN PEXCEPTION_RECORD ExceptionRecord,
    ...
    IN BOOLEAN SecondChance)
{
    ...
    if (SecondChance)
    {
        Global::PsSetProcessFaultInformation(Process, (int*)&v13);
    }
    ...
    if (IsUseDebugPort)  //是否将异常转发到调试端口
    {
        ...
        {
            Port = DebugProcess->DebugObject;
            LOG_DEBUG("DbgkDebugObject: %llX", DebugProcess->DebugObject);
        }
        else
        {
            size_t ptr_DebugPort = (size_t)Process + Offset::EProcess::DebugPort;
    ...
    else
    {
        ...
        UseLpc = TRUE;
        Port = Global::PsCaptureExceptionPort(Process);
    }
    ...
    if (IsDebugTargetProcess(Process, &DebugProcess))
    {
            ...
            (ExceptionRecord->ExceptionCode == STATUS_WX86_SINGLE_STEP))
        {
            ...
            PETHREAD Thread = (PETHREAD)PsGetCurrentThread();
            size_t kthread_base = (size_t)Thread + Offset::EThread::Tcb;
            size_t ptr_TrapFrame = kthread_base + Offset::KThread::TrapFrame;
    ...
    Global::KeCopyExceptionRecord(&ApiMessage.u.Exception.ExceptionRecord, ExceptionRecord);
    ...
    if (UseLpc)
    {
        ...
        {
            Status = Global::DbgkpSendApiMessageLpc(&ApiMessage, Port, IsUseDebugPort);
    ...
    else
    {
        ...
        Status = Global::DbgkpSendApiMessage(Process, IsUseDebugPort != 0, &ApiMessage);
    }
    ...
    if (ApiMessage.ReturnedStatus == DBG_EXCEPTION_NOT_HANDLED)
    {
        if (!IsUseDebugPort)
        {
            Status = Global::DbgkpSendErrorMessage(ExceptionRecord, 2, &ApiMessage);
...
//NTSTATUS
//Global::DbgkClearProcessDebugObject(
//    IN PEPROCESS Process,
...
//    if (NT_SUCCESS(Status))
//    {
//        Global::DbgkpMarkProcessPeb(Process);
//    }
//
//    //
...
--*/
{
    ...
    //if (PreviousMode == UserMode && !(((PEPROCESS)PsGetCurrentProcess())->Flags2 & PS_PROCESS_FLAGS2_PROTECTED_PROCESS))
    //{
    ...
    if (NT_SUCCESS(Status)) {
        Status = Global::DbgkClearProcessDebugObject(Process, DebugObject);
        DeleteDebugProcess(DebugObject);
        DbgPrint("Global::DbgkClearProcessDebugObject Status: %x", Status);
        ObDereferenceObject(DebugObject);
    }
...
NTSTATUS DbgkpSetProcessDebugObject(IN PEPROCESS Process,
    ...
    IN PETHREAD LastThread)
{
    ...
    if (NT_SUCCESS(Status))
    {
        for (;;)
        {
            ...
            else
            {
                ...
                /* Get the next thread */
                ThisThread = Global::PsGetNextProcessThread(Process, LastThread);
                if (ThisThread)
                {
                    ...
                    *(PDEBUG_OBJECT*)ptr_DebugPort = NULL;
                    ...
                    /* Post fake messages */
                    Status = Global::DbgkpPostFakeThreadMessages(Process,
                        DebugObject,
                        ThisThread,
                        &FirstThread,
                        &LastThread);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        else
        {
            ...
            if (!IsDebugTargetProcess(Process, &DebugProcess))
            {
                size_t ptr_TargetFlags = (size_t)Process + Offset::EProcess::Flags;
    ...
    while (NextEntry != &DebugObject->EventList)
    {
            ...
            (DebugEvent->BackoutThread == CurrentThread))
        {
            ...
            if (DebugEvent->Flags & DEBUG_EVENT_RELEASE)
            {
                ...
                size_t ptr_RundownProtect = (size_t)EventThread + Offset::EThread::RundownProtect;
    ...
    if (GlobalHeld) ExReleaseFastMutex(&Global::DbgkpProcessDebugPortMutex);
    ...
    if (NT_SUCCESS(Status)) Global::DbgkpMarkProcessPeb(Process);
    return Status;
}

//附加调试进程
EXTERN_C
NTSTATUS Fake_NtDebugActiveProcess(IN HANDLE ProcessHandle,
    IN HANDLE DebugHandle)
{
    NTSTATUS Status; // eax
    PETHREAD LastThread; // [rsp+30h] [rbp-18h]
    PEPROCESS TargetProcess; // [rsp+60h] [rbp+18h]
    ...
    if (NT_SUCCESS(Status))
    {
        if (TargetProcess == (PEPROCESS)PsGetCurrentProcess() || TargetProcess == (PEPROCESS)PsInitialSystemProcess)
        {
        ...
        {
            PEPROCESS CurrentProcess = (PEPROCESS)PsGetCurrentProcess();
            size_t ptr_CurrentWoW64Process = (size_t)CurrentProcess + Offset::EProcess::WoW64Process;
            unsigned short currentMachine = WeDebug_GetProcessMachine(CurrentProcess);
            size_t ptr_TargetWoW64Process = (size_t)TargetProcess + Offset::EProcess::WoW64Process;
            unsigned short targetMachine = WeDebug_GetProcessMachine(TargetProcess);

            if ((!*(size_t*)ptr_CurrentWoW64Process) ||
                (currentMachine != IMAGE_FILE_MACHINE_I386 && currentMachine != IMAGE_FILE_MACHINE_ARMNT) ||
                (*(size_t*)ptr_TargetWoW64Process && (targetMachine == IMAGE_FILE_MACHINE_I386 || targetMachine == IMAGE_FILE_MACHINE_ARMNT)))
            {
                ...
                if (NT_SUCCESS(Status))
                {
                    ...
                    size_t ptr_RundownProtect = (size_t)TargetProcess + Offset::EProcess::RundownProtect;
                    if (ExAcquireRundownProtection((PEX_RUNDOWN_REF)ptr_RundownProtect))
                    {
                        Status = Global::DbgkpPostFakeProcessCreateMessages(TargetProcess, DebugObject, &LastThread);
...
//VOID
//NTAPI
//Global::DbgkpConvertKernelToUserStateChange(OUT PDBGUI_WAIT_STATE_CHANGE WaitStateChange,
...
//    LOG_DEBUG("事件: %d", DebugEvent->ApiMsg.ApiNumber);
...
//VOID
//NTAPI
//Global::DbgkpOpenHandles(IN PDBGUI_WAIT_STATE_CHANGE WaitStateChange,
//    IN PEPROCESS Process,
//    IN PETHREAD Thread)
//{
//    NTSTATUS Status;
...
//    {
//        /* Duplicate it */
//        Status = Global::ObDuplicateObject((PEPROCESS)PsGetCurrentProcess(),
//            Handle,
...
//取出调试事件
EXTERN_C
NTSTATUS Fake_NtWaitForDebugEvent(IN HANDLE DebugHandle,
    ...
    while (TRUE)
    {
        ...
        else
        {
            ...
            if (GotEvent)
            {
                ...
                Global::DbgkpConvertKernelToUserStateChange(&WaitStateChange, DebugEvent);
        ...
        else
        {
            ...
            Global::DbgkpOpenHandles(&WaitStateChange, Process, Thread);  //获得进线程句柄返回给Ring3
    ...
    --*/
{
    ...
    if (GotEvent)
    {
        ...
        Global::DbgkpWakeTarget(FoundDebugEvent);
    }
    else
    {
...
EXTERN_C
VOID
Fake_DbgkMapViewOfSection(IN PEPROCESS Process,
    ...
    IN ULONG_PTR ViewSize)
{
    ...
    size_t kthread_base = (size_t)Thread + Offset::EThread::Tcb;
    size_t ptr_PreviousMode = kthread_base + Offset::KThread::PreviousMode;
    if (*(char*)ptr_PreviousMode)
    {
        //if (!(Thread->CrossThreadFlags & PS_CROSS_THREAD_FLAGS_HIDEFROMDBG))
        {
            ...
            if (DebugPort)
            {
                ...
                //else
                {
                    size_t ptr_Teb = kthread_base + Offset::KThread::Teb;
                    Teb = *(PTEB*)ptr_Teb;
                }

                size_t ptr_Process = kthread_base + Offset::KThread::Process;
                if (Teb && (Process == *(PEPROCESS*)ptr_Process))
                {
                    if (Global::DbgkpSuppressDbgMsg(Teb))
                        return;
                ...
                LoadDllArgs->FileHandle = Global::DbgkpSectionToFileHandle(SectionObject);
                ...
                __try
                {
                    ...
                    if (NtHeaders)
                    {
                        size_t ptr_FileHeader = (size_t)NtHeaders + Offset::ImageNtHeaders64::FileHeader;
                        size_t ptr_PointerToSymbolTable  = ptr_FileHeader + Offset::ImageFileHeader::PointerToSymbolTable;
                        size_t ptr_NumberOfSymbols = ptr_FileHeader + Offset::ImageFileHeader::NumberOfSymbols;
                ...
                Global::DbgkpSendApiMessage(Process, TRUE, &ApiMsg);
...
EXTERN_C
VOID
Fake_DbgkUnMapViewOfSection(IN PEPROCESS Process, IN PVOID BaseAddress)
{
    PETHREAD Thread; // r8
    PTEB Teb; // rcx
    DBGKM_APIMSG ApiMsg = { 0 }; // [rsp+20h] [rbp-118h]
    ...
    {        
        //if (!(Thread->CrossThreadFlags & PS_CROSS_THREAD_FLAGS_HIDEFROMDBG))
        {
            ...
            if (DebugPort)
            {
                ...
                size_t ptr_Process = kthread_base + Offset::KThread::Process;
                if (!Teb || (Process != *(PEPROCESS*)ptr_Process) || (!Global::DbgkpSuppressDbgMsg(Teb)))
                {
...

{
    ...
    size_t ptr_CrossThreadFlags = (size_t)Thread + Offset::EThread::CrossThreadFlags;
...

{
    ...
    if (CrossThreadFlags.ThreadInserted)
    {
        size_t ptr_ExitTime = (size_t)Process + Offset::EProcess::ExitTime;
        ...
        Global::DbgkpSendApiMessage(Process, FALSE, &ApiMsg);        
        PrintProcessName(Process);
    }
}