...
#include "KernelCommon.Encrypt.Blowfish.h"
...
VOID DecryptData(PVOID pInAddr, PVOID pOutAddr)
{
	...
	//一组明文对应一组key
	RtlStringCchCopyA((NTSTRSAFE_PSTR)aucKey, sizeof(aucKey), key);
	RtlStringCchLengthA((STRSAFE_PCNZCH)pInAddr, NTSTRSAFE_MAX_CCH * sizeof(char), &aucCipherTextLen);
...
VOID TestBlowfish()
{
	...
	for (ULONG i = 0; i < 34; i++)
	{
		...
		//一组明文对应一组key
		RtlStringCchCopyA((NTSTRSAFE_PSTR)aucKey, sizeof(aucKey), szData[i][0]);
		RtlStringCchCopyA((NTSTRSAFE_PSTR)aucPlainText, sizeof(szData[i][1]), szData[i][1]);
		BlowFishInit(&oBlowFish, aucKey, 16);
		memset(aucCipherText, 0x00, sizeof(aucCipherText));
		RtlStringCchLengthA((STRSAFE_PCNZCH)aucPlainText, NTSTRSAFE_MAX_CCH * sizeof(char), &aucPlainTextLen);
		...
		RtlStringCchLengthA((STRSAFE_PCNZCH)aucCipherText, NTSTRSAFE_MAX_CCH * sizeof(char), &aucCipherTextLen);
