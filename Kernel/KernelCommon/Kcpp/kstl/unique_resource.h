    ...
    namespace experimental 
    {
        ...
        public:
            ...
            explicit unique_resource_t(R && resource, D && deleter, bool shouldrun = true) UNIQUE_RESOURCE_NOEXCEPT
                : resource(eastl::move(resource))
                , deleter(eastl::move(deleter))
                , execute_on_destruction{ shouldrun }
            {}

            // move
            unique_resource_t(unique_resource_t &&other) UNIQUE_RESOURCE_NOEXCEPT
                : resource(eastl::move(other.resource))
                , deleter(eastl::move(other.deleter))
            ...
            void reset(R && newresource) UNIQUE_RESOURCE_NOEXCEPT
            {
                ...
                resource = eastl::move(newresource);
            }
            ...
            eastl::add_lvalue_reference_t<eastl::remove_pointer_t<R>> operator*() const
            {
                return *resource;
            }
        ...
        {
            return unique_resource_t<R, D>(eastl::move(r), eastl::move(t), true);
        }
        ...
        unique_resource_t<R, D> unique_resource_checked(R r, R invalid, D t) UNIQUE_RESOURCE_NOEXCEPT
        {
            ...
            return unique_resource_t<R, D>(eastl::move(r), eastl::move(t), shouldrun);
        }
    }
}
