    ...
    namespace experimental
    {
		...
		struct scope_exit
		{
			...
			explicit
				...
				: exit_function(eastl::move(f))
				, execute_on_destruction{ true }
			{
			}
			// move
			scope_exit(scope_exit&& rhs) SCOPE_GUARD_NOEXCEPT()
				: exit_function(eastl::move(rhs.exit_function))
		...
		template <typename EF>
		auto make_scope_exit(EF&& exit_function) -> decltype(scope_exit<eastl::remove_reference_t<EF>>(eastl::forward<EF>(exit_function))) SCOPE_GUARD_NOEXCEPT()
		{
			return scope_exit<eastl::remove_reference_t<EF>>(eastl::forward<EF>(exit_function));
		}
