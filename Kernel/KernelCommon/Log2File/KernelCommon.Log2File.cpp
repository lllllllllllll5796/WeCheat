...
#include "KernelCommon.Log2File.h"
...
namespace KernelCommon
{
	...
	NTSTATUS WriteLogToXmlFileW(const wchar_t* logMessage)
	{
		...
		if (!NT_SUCCESS(status))
		{
			LOG_ERROR("ZwCreateFile日志文件失败!\r\n");
			return status;
		}

		status = ZwWriteFile(fileHandle, NULL, NULL, NULL, &ioStatus, (PVOID)logMessage, (ULONG)wcslen(logMessage) * sizeof(WCHAR), NULL, NULL);
		if (!NT_SUCCESS(status))
		{
			LOG_ERROR("ZwWriteFile日志文件失败!\r\n");
			ZwClose(fileHandle);
			return status;
		}
	...
	NTSTATUS WriteLogToXmlFileA(const char* logMessage)
	{
		...
		status = ZwWriteFile(fileHandle, NULL, NULL, NULL, &ioStatus, (PVOID)logMessage, (ULONG)strlen(logMessage), NULL, NULL);
		if (!NT_SUCCESS(status))
		{
			LOG_ERROR("ZwWriteFile日志文件失败!\r\n");
			ZwClose(fileHandle);
	...
	NTSTATUS InitDriverLog(const wchar_t* LogPath)
	{
		...
		// 创建Logs目录
		status = KernelCommon::CreateLogsDirectory(oxorany(L"\\??\\C:\\Logs"));
		if (!NT_SUCCESS(status))
		{
			LOG_ERROR("创建目录失败!\r\n");
			return status;
		...
		eastl::wstring LogFilePath = oxorany(L"\\??\\");
		LogFilePath += LogPath;

		//创建日志文件
		status = KernelCommon::SetLogFilePath(LogFilePath.c_str());
		if (!NT_SUCCESS(status))
		{
			LOG_ERROR("设置日志文件失败!\r\n");
			return status;
	...
	VOID RemovePath(wchar_t* fullPath)
	{
